---
name: dev-baseline
description: Initialize project baselines, review backlog, suggest improvements, plan new requirements, execute work only after confirmation, and publish confirmed repository changes with git add, commit, and push when explicitly requested. Use to inspect docs coverage, summarize remaining work, propose project improvements, create structured task plans, manage implementation, and safely commit/push completed work through a documentation-first workflow.
...
- Do not treat execution confirmation such as `开始工作` or `start` as permission to commit or push.
- Do not treat git publishing as permission to edit source files.
...
## Git publish output format

At the end of Mode E, respond in this structure:
...
If the user has not explicitly triggered Git publish mode, do not run `git add`, `git commit`, or `git push`.
