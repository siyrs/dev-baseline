...
for file in "${required[@]}"; do
  [[ -f "$workspace/$file" ]] || fail "Missing compact traceability file: $file"
done
...
if [[ ${#errors[@]} -eq 0 ]]; then
  ...
  [[ ${#fp_ids[@]} -gt 0 ]] || fail "No function points found in 01-task-contract.md."
  [[ ${#ac_ids[@]} -gt 0 ]] || fail "No acceptance criteria found in 01-task-contract.md."
